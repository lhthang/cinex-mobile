...
class EditShowtimeView extends StatefulWidget {
  final Showtime showtime;

  EditShowtimeView({Key key,@required this.showtime}) : super(key:key);
  @override
  _EditShowtimeViewState createState() => _EditShowtimeViewState();
}

class _EditShowtimeViewState extends State<EditShowtimeView> {
  ...
  void initState() {
    ...
    super.initState();
    _idController.text=widget.showtime.id;
    _priceController.text=widget.showtime.price.toString();
    timeOfDay = getTimeFromString(widget.showtime.startAt);
    selectedDate = getDateTimeFromString(widget.showtime.startAt);
  ...
  _createSelectRoom(List<Room> roomsList){
    if(selectedRoom == null){
      for (var room in roomsList){
        if(room.id == widget.showtime.room.id){
          selectedRoom=room;
          break;
        }
      }
  ...
  _createSelectMovie(List<Movie> movieList){
    if (selectedMovie==null){
      for (var movie in movieList){
        if (movie.id==widget.showtime.movieId) {
          selectedMovie = movie;
          break;
        }
      }
  ...
  _createSelectScreenType(List<ScreenType> screenTypesList){
    if (selectedScreenType==null){
      for (var screenType in screenTypesList){
        if (screenType.id==widget.showtime.screenType.id) {
  ...
  _updateShowtime() async{
    showDialog(
        context: context,
        builder: (BuildContext context) {
          return AlertDialog(
            title: new Text('Confirm'),
            content: new Text('Do you want to update this showtime ?', style: contentStyle),
            actions: <Widget>[
              new FlatButton(
                child: new Text('Ok',),
                onPressed: () async {
                  ...
                  showtimeReq = new ShowtimeRequest(movieId: selectedMovie.id,screenTypeId: selectedScreenType.id,
                      startAt: startAt.toIso8601String(),roomId: selectedRoom.id,price:double.tryParse(_priceController.text) );
                  Navigator.of(context).pop();
                  if(isSupport==false)
                    dialog.showErrorMessage(this.context,"Error", 'Screen type of showtime is not suitable for movie and room.' + '\nPlease try again!').show();
                  if (showtimeReq.movieId != '' &&showtimeReq.screenTypeId!=''
                      &&showtimeReq.startAt!=null&& showtimeReq.roomId!=''&&showtimeReq.price!=0) {
                    if (await ShowtimeController.instance.updateShowtime(widget.showtime.id,showtimeReq) ){
                      dialog.showSuccessMessage(
                          this.context, "Success", "Update showtime successfully!").show();
                    } else
                      dialog.showErrorMessage(this.context,"Error", 'Update showtime failed.' + '\nPlease try again!').show();
                    return;
                  }
