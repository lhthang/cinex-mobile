...
class _EditUserViewState extends State<EditUserView> {
  ...
  Map<Role, bool> values = {
  };

  Map<Role, bool> previousValues = {
  };
                ...
                child: InkWell(
                  ...
                  onTap: () {
                    for (var role in snapshot.data){
                      values[role] =  false;
                      for(var userRole in widget.user.roles){
                        if (userRole.id == role.id.toString() )
                          values[role] = true;
                        else
                          continue;
                      }
  ...
  _getSelectedRole(){
    List<Role> ids=new List();
    for(var sc in values.keys){
      if(values[sc]==true){
        ids.add(sc);
      }
    }
          ...
          return AlertDialog(
                ...
                return SingleChildScrollView(
                  ...
                  child: ListTileTheme(
                    ...
                    child: Column(
                      ...
                      children: values.keys.map((Role role){
                        return Row(
                          mainAxisSize: MainAxisSize.min,
            ...
            actions: <Widget>[
              FlatButton(
                child: Text('CANCEL'),
                onPressed: (){
                  if (!mapEquals(values, previousValues)){
                    values=new Map.from(previousValues);
                  }
              ...
              FlatButton(
                ...
                onPressed: (){
                  setState(() {
                    previousValues= new Map.from(values);
                    selectedRoles=_getSelectedRole();
                  });
